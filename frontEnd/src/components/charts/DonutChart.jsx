...
const CustomLegend = ({ labels, values, colors, hiddenIndexes, toggleIndex }) => {
  const totalRevenue = values.reduce((sum, val, i) => sum + (hiddenIndexes[i] ? 0 : val), 0);
    ...
    <div className="flex flex-col space-y-3 p-6 border border-gray-100 rounded-2xl bg-white shadow-lg hover:shadow-xl transition duration-300 w-full md:max-w-sm">
      ...
      <ul className="space-y-2">
        {labels.map((label, i) => {
          ...
          const percentage = totalRevenue ? ((value / totalRevenue) * 100).toFixed(2) : 0;
          return (
            <li
              key={label}
...
function DonutChart({ labels = [], dataPoints = [], colors = [] }) {
  ...
  // Prepare chart data dynamically, hiding selected slices
  ...
  const options = useMemo(() => ({
    ...
    plugins: {
      ...
      datalabels: {
        ...
        formatter: (value) => {
          if (!value) return "";
          const percentage = totalRevenue ? ((value / totalRevenue) * 100).toFixed(1) : 0;
          return `→ ${percentage}%`;
        },
      },
    },
